...
**Extraction quality.** The risk area. Different provider formats will stress the prompt differently. Mitigations: (1) validate returned JSON against a Zod schema, (2) persist `Extraction.raw_json` so we can re-parse with a better prompt without re-spending, (3) UI shows the source PDF alongside extracted values for spot-checking, (4) `pnpm compare-extract <pdf-path-or-report-id>` runs the deterministic dispatcher and Claude on the same PDF and prints a diff (matched / value mismatches / units mismatches / only-in-each) — used to validate new deterministic parsers against Claude's output as a ground truth.

**Deterministic parser strategies (in escalating cost / accuracy).** Today's deterministic parsers (`lib/parsers/<provider>/`) extract the PDF text via pdfjs and walk it as a stream of "rows" reconstructed from text-item Y-positions, then run a small state machine that maps section headers + row shapes to metric records. This works for tabular reports whose layout pdfjs can recover cleanly, and is what the GI-MAP parser does today. When that approach hits its limits — heavily templated layouts, multi-column grids that pdfjs splits unpredictably, or visual elements like donut-chart legends that aren't real text rows — the next step is **template-aware extraction**: hard-code each panel as a (page, x-range, y-range) zone and pull the text inside that zone directly from pdfjs. Highest accuracy on a single template version, near-zero state-machine complexity, but breaks when the lab redesigns the report. Worth reaching for only if the row-stream approach is bottoming out on a high-volume format; default to the row-stream approach for new parsers because it tolerates minor layout drift without code changes.
